...
1. Run `node tools/cli.ts pick` to get today's ten questions as JSON.
...
4. Record the results with
   `node tools/cli.ts answer <id>:1 <id>:0 ...` — `1` for right, `0` for wrong,
   using the `word.id` from the JSON.
5. Finish with `node tools/cli.ts stats` and a one-line read on how it went and
